...
<form action="{{ route('Panel.blogs.update',$id) }}" method="POST" id="blogForm" enctype="multipart/form-data" class="w-100">
    @csrf
    ...
    <div class="form-group col-md-6">
        ...
        <img class="mt-3" src="{{ asset('images/coverBlog/'.$blog->cover_image) }}" style="border-radius: 5px" width="100px" height="70px" alt="avatar">
    </div>
    <div class="form-group col-md-6">
        <label>slug:</label>
        <input class="form-control" id="slug" type="slug" name="slug" value="{{ old('slug',$blog->slug ?? '') }}">
    </div>
    </div>
    <div class="form-group">
        <label>متن:</label>
    <textarea name="content" id="summernote">{{ old('content',$blog->content ?? '') }}</textarea><br>
...
{!! JsValidator::formRequest('App\Http\Requests\blog\blogEditRequest', '#blogForm'); !!}
{{-- Summernote CSS --}}
